...
@section('panel')
    <div class="col-md-6 col-sm-12">
        <h3 class="animated fadeInLeft">{{__('Subject')}}</h3>
    </div>
    <div class="col-md-6 col-sm-12">
        <div class="pull-right">
            @can('subject-create')
                <h3 class="animated fadeInRight"><a class="btn btn-success"
                                                    href="{{ route('modules.subjects.create') }}"> {{__('Create New Subject')}}</a>
                </h3>
            @endcan
        </div>
    </div>
            ...
            <div class="responsive-table">
                <table class="table table-striped table-bordered" id="subject-table" style="width:100%">
                    ...
                    <tr>
                        ...
                        <th data-title="{{__('Subject case')}}">{{__('Subject case')}}</th>
                        <th data-title="{{__('Nicknames')}}">{{__('Nicknames')}}</th>
...
@section('script')
    <script>
        $('#subject-table').DataTable({
            "processing": true,
            "serverSide": true,
            "ajax": '{{route('modules.subjects.index')}}',
            "columns": [
                {data: 'id', name: 'id'},
