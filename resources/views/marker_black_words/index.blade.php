...
@section('panel')
    <div class="col-md-6 col-sm-12">
        <h3 class="animated fadeInLeft">{{__('Black word markers')}}</h3>
    </div>
    <div class="col-md-6 col-sm-12">
        <div class="pull-right">
            @can('marker-black-word-list')
            <h3 class="animated fadeInRight"><a class="btn btn-success" href="{{ route('modules.marker_black_words.create') }}"> {{__('Create New Word')}}</a></h3>
            @endcan
        </div>
    </div>
@endsection
    ...
    <div class="panel">
        <div class="panel-body">
            <div class="responsive-table">
                <table class="table table-striped table-bordered" id="marker-black-word-table" style="width:100%">
                <thead>
                    <tr>
                        <th data-title="{{__('No')}}" width="50">{{__('No')}}</th>
                        <th data-title="{{__('Terminology')}}">{{__('Terminology')}}</th>
...
@section('script')
<script>
    $('#marker-black-word-table').DataTable( {
        "processing": true,
        "serverSide": true,
        "ajax": '{{route('modules.marker_black_words.index')}}',
        "columns": [
            {data: 'id', name: 'id'},
            {data: 'terminology', name: 'terminology'},
            {data: 'word', name: 'word'},
            {data: 'declension', name: 'declension'},
